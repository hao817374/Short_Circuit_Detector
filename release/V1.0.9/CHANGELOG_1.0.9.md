...
- **空间校准界面功能与视觉完善**:
    ...
    - 方向校准状态文字简化：移除"部分成功"，任一失败显示"校准失败"，都没有失败显示"校准成功"。
...
- **表笔断开判定优化**:
    - `probeThreshold` 默认值 -4300 → -3000。
    - Compass.tsx 恢复使用 `probeThreshold` prop（此前硬编码为 -3000，未使用 Settings 配置值）。
    - 系统配置界面新增"表笔断开判定"输入框（`type="number"`），用户可直接输入自定义阈值。
    - 保留过载保护逻辑：任一通道 |raw| > 25000 视为断开。

- **DEBUG 视图新增波形范围显示**:
    - Col 3 新增第 4 行"波形范围"，实时显示当前帧 min / max / Δ（极差），用于观察悬空与接触 PCB 时的波形特征差异。
